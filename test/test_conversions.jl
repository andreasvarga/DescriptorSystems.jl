...
sysc = dss(a,e,b,c,d);
@time sysd, xd0, Mx, Mu = c2d(sysc,1; x0, u0, state_mapping = true, simple_infeigs = false); # state-mapping not feasible due to uncontrollable infinite eigenvalue
@test isnothing(Mx)
...
@testset "timeresp" begin
      ...
      A, B, C, D, xt, Mx, Mu = DescriptorSystems.state_map(sysd, x0; state_mapping = true)  
      @time y1, tout1, x1 = timeresp(dss(A,B,C,D,Ts=-1), u, Int[], xt; state_history = true, atol=1.e-7)
      @test norm(y-y1,Inf) < 1.e-7 && [Mx Mu]*[x1';u'] ≈ x'

      @time y2, tout2, x2 = timeresp(dss(A,B,C,D,Ts=-1), u, Int[]; state_history = true, atol=1.e-7)
      @time y3, tout3, x3 = timeresp(sysdr, u, Int[]; state_history = true, atol=1.e-7)
